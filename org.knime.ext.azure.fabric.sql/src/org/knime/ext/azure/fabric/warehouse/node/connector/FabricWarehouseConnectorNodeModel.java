...
final class FabricWarehouseConnectorNodeModel extends WebUINodeModel<FabricWarehouseSettings> {
    ...
    protected PortObject[] execute(final PortObject[] inObjects, final ExecutionContext exec,
        ...
        // final FabricWarehouseSettings settings = getSettings().orElseThrow();
        ...
        final var spec2 = ((CredentialPortObject) inObjects[1]).getSpec();
        JWTCredential credential = spec2.resolveCredential(JWTCredential.class);

        final DBDriverWrapper driver = getDriver();
        final DBConnectionController controller = createConnectionController(settings, connection, credential);
    ...
    private static DBDriverWrapper getDriver() throws InvalidSettingsException {
        final var latestDriver = DBDriverRegistry.getInstance().getLatestDriver(SQLSERVER_TYPE);
    ...
    private static DBConnectionController createConnectionController(final FabricWarehouseSettings settings,
            final FabricConnection connection, final JWTCredential credential)
        ...
        // connection.getCredential().resolveCredential(JWTCredential.class);
        return new MSAuthDBConnectionController(credential, jdbcUrl);
    }
