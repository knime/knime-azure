...
public class AzureBlobStorageConnectorNodeParameters implements NodeParameters {
	...
	@Widget(title = "Working directory", //
			description = """
					Specifies the <i>working directory</i> using the path syntax explained above. The working directory \
					must be specified as an absolute path. A working directory allows downstream nodes to access \
					files/folders using <i>relative</i> paths, i.e. paths that do not have a leading slash. If not \
					specified, the default working directory is <tt>/</tt>.""")
	...
	@Widget(title = "Normalize paths", //
			description = """
					...
					redundant components like <tt>../</tt> or <tt>.</tt> are part of an existing object, then normalization \
					must be deactivated in order to access them properly.""")
	@Layout(FileSystemSettingsSection.class)
	@ValueReference(NormalizePathsRef.class)
	@Persist(configKey = "normalizePaths")
	...
	public void validate() throws InvalidSettingsException {
		...
		if (!m_workingDirectory.startsWith(AzureBlobStorageFileSystem.PATH_SEPARATOR)) {
	...
	static final class FileSystemConnectionProvider implements StateProvider<FSConnectionProvider> {
		...
		private static AzureBlobStorageFSConnection toFSConnection(final AzureBlobStorageFSConnectionConfig config)
			...
			AzureBlobStorageConnectorNodeModel.testConnection(connection);
			return connection;
		}
	}
}
