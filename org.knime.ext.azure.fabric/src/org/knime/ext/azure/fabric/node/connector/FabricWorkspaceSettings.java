...
public class FabricWorkspaceSettings implements DefaultNodeSettings {
    ...
    @Widget(title = "Microsoft Fabric workspace", //
            description = """
                    Select the \
                    <a href='https://learn.microsoft.com/fabric/fundamentals/workspaces'>Microsoft Fabric workspace</a>\
                    to work with.
                    """)
    ...
    @Widget(title = "Read timeout (seconds)",
        description = "Timeout in seconds to read data from an established connection,"
                    + " or 0 for an infinite timeout. Used by this and downstream nodes connecting to Microsoft Fabric.", //
        advanced = true)
    @NumberInputWidget(validation = IsNonNegativeValidation.class)
    @Layout(ConnectionTimeoutsSection.class)
    ...
    private void validateWorkspaceId() throws InvalidSettingsException {
        if (StringUtils.isBlank(m_workspaceId)) {
            throw new InvalidSettingsException("Select the Microsoft Fabric workspace to work with.");
        }
    }
    ...
    private static final class WorkspaceChoiceProvider implements StringChoicesProvider {
        ...
        public List<StringChoice> computeState(final DefaultNodeSettingsContext context) {
            try {
                final WorkspaceAPI client = FabricRESTClient //
                        .fromCredentialPort(WorkspaceAPI.class, context.getPortObjectSpecs(),
                                Duration.ofSeconds(10), Duration.ofSeconds(10));
                //// TK_TODO: How to access the user input values here similar to
                //// DBTypeStateProvider

                final List<Workspace> workspaces = getAllWorkspaces(client);
        ...
        private static List<Workspace> getAllWorkspaces(final WorkspaceAPI client) throws IOException {
            final LinkedList<Workspace> results = new LinkedList<>();
            ...
            do {
                ...
                if (StringUtils.isEmpty(continuationToken)) {
                    ...
                    workspaces = client.listWorkspaces(continuationToken);
                }
                results.addAll(Arrays.asList(workspaces.workspaces));
                continuationToken = workspaces.continuationToken;
