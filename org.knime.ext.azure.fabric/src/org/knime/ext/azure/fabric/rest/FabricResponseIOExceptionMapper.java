 ...
 * </ul>
 */
class FabricResponseIOExceptionMapper implements ResponseExceptionMapper<IOException> {
    ...
    public IOException fromResponse(final Response response) { // NOSONAR it's simple enough
        ...
        if ((response.getStatus() == 401 || response.getStatus() == 403) && !StringUtils.isBlank(message)) {
            ...
            return new FabricRateLimitException(response.getHeaderString("retry-after"), message);
        } else if (response.getStatus() == 429) {
            return new FabricRateLimitException(response.getHeaderString("retry-after"));
