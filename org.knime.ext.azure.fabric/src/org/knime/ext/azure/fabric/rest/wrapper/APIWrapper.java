...
public class APIWrapper<T> {
    ...
    private static final NodeLogger m_logger = NodeLogger.getLogger(FabricRESTClient.class); // NOSONAR use a more specific logger name instead of APIWrapper
    ...
    protected <R> R invoke(final Invoker<R> invoker) throws IOException {
        for (int i = 0; i < Integer.MAX_VALUE; i++) {
            ...
            } catch (final FabricRateLimitClientErrorException e) { // NOSONAR
                waitBeforeRetry(i, e.getRetryAfter().orElse(null));
            } catch (final FabricRateLimitException e) { // NOSONAR
                waitBeforeRetry(i, e.getRetryAfter().orElse(null));
            }
        }

        throw new IOException("To many retries, Microsoft Fabric request failed with rate limit."); // shoud never be
                                                                                                    // reached
    }
    ...
    private void waitBeforeRetry(final int retryRound, final Duration retryAfter) throws IOException {
        ...
        if (retryRound % 10 == 0) {
            m_logger.debug(String.format(
                    "Microsoft Fabric REST API returned rate-limit error, waiting %d seconds before next retry."
                    + " (retry: %d, API: %s)",
        ...
        try {
            ...
            throw new IOException("Microsoft Fabric REST request interrupted while waiting for retry.", ex);
        }
    }
